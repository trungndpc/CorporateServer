...
public class MessageManager {
    ...
    private static final String TMP_REJECTED_PROMOTION = "Đơn tham gia chương trình khuyến mãi %s không được xác nhận thành công. Lý do: %s.Mọi thắc mắc xin liên hệ hotline 18001718. Click vào link dưới để cập nhật lại: %s";
    ...
    public static String getMsgRejectedPromotion(String titlePromotion, String note, String link) {
        return String.format(TMP_REJECTED_PROMOTION, titlePromotion, note, link);
    }
    ...
    public static String getLink2UpdateConstruction(int promotionId, int constructionId, TypeConstruction typeConstruction) {
        if (typeConstruction == TypeConstruction.NOW_CONSTRUCTION) {
            return String.format("%s/khuyen-mai/%d/up-hoa-don-nha-qua/%d", Constant.CLIENT_DOMAIN, promotionId, constructionId);
        }else {
            return String.format("%s/khuyen-mai/%d/cong-trinh-tiep-theo/%d", Constant.CLIENT_DOMAIN, promotionId, constructionId);
        }
    }
